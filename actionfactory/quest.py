...
class Quest:
	def __init__(self,city,castleid=None):
		...
		self.qcompleted=[]
	def completequest(self,questid,castleid=None,wait=True,tried=0,qtype=1):
		if tried==60:
			return
		if castleid==None:
			castleid=self.castleid
		self.city.client.sendmessage('quest.getQuestType',{'castleId':castleid,'type':qtype})
		...
		if questid in self.qcompleted:
			...
			if wait:
				time.sleep(1)
				self.completequest(questid,castleid,tried=(tried+1),qtype=qtype)
				return
	def special(self):
		while True:
